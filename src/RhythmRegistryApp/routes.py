...
# ---------------------- CONSTANTS ---------------------- #
PER_PAGE_PORTRAITS = 5
PER_PAGE_ROLES = 5
PER_PAGE_ROLES_IN_PORTRAITS = 5
...
def delete_portrait_in_role(pir_id):
    ...
    # Get all relationships for this portrait
...
def delete_all_portraits():
    ...
    # Then delete portraits
    Portrait.query.delete()
    
    # Reset the auto-increment counters using a more reliable approach
    ...
    # Go back to database management page
...
def delete_all_roles():
    ...
    # Reset the auto-increment counter for roles
    from sqlalchemy import text
...
def import_wikipedia():
    ...
    # Initialize errors dictionary to collect all validation errors
    errors = {}
    
    # Get month and day values - check both possible field names
    if use_date_range:
        start_month = request.form.get('start_month', '')
        start_day = request.form.get('start_day', '')
    else:
        # For single date, use the wiki_month and wiki_day fields
    ...
    # If using date range, get end date, otherwise use start date for both
    if use_date_range:
    ...
    # Path to the data_import folder and script
    ...
    app.logger.info(f"Running command: {cmd_command}")
    
    # Execute the command
    os.system(cmd_command)
    
    # Create message for user
    date_range_msg = f" to {end_date}" if use_date_range and end_date != start_date else ""
    sections_msg = ""
    if import_births and import_deaths:
        sections_msg = " (births and deaths)"
    elif import_births:
        sections_msg = " (births only)"
    elif import_deaths:
        sections_msg = " (deaths only)"
    
    flash(f"Wikipedia data import script is now running for dates {start_date}{date_range_msg}{sections_msg}. Please wait for it to complete.", "info")
    return redirect(url_for("manage_database"))
