...
class Portrait(db.Model):
    """Portrait model representing historical music figures"""
    id = db.Column(db.Integer, primary_key=True) 
...
class Role(db.Model):
    """Role model representing musical roles or professions"""
    id = db.Column(db.Integer, primary_key=True)
...
class PortraitsInRoles(db.Model):
    """Association model connecting portraits with their secondary roles"""
    id = db.Column(db.Integer, primary_key=True)
